...
/**
 * The Appointment controller class builds and populates the <code>Appointment</code>> tableview. Navigation buttons are
...
public class AppointmentController implements Initializable {
    ...
    /**
     * Initializes the <code>Appointment</code> tableview with all appointments in the database. A ChangeListener is used to
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        ObservableList<Appointment> apptAllList = AppointmentOBJ.getAllAppointments();
        ...
        apptListViewTG.selectedToggleProperty().addListener((observableValue, toggle, t1) -> {
            ...
            if (selectedButton == rbtnWeekView) {
                ObservableList<Appointment> apptWeekList = AppointmentOBJ.getCurrentWeekAppts();
            ...
            if (selectedButton == rbtnMonthView) {
                ObservableList<Appointment> apptMonthList = AppointmentOBJ.getCurrentMonthAppts();
            ...
            if (selectedButton == rbtnAllAppts) {
                ObservableList<Appointment> apptAllList1 = AppointmentOBJ.getAllAppointments();
    ...
    public void toUpdateAppt(ActionEvent actionEvent) throws IOException {
        ...
        if (a != null) {
            UpdateAppointmentController.passingTheAppointment(a);
            Scenemovement.goToPage(actionEvent, "UpdateAppointments", 600, 550, "Update Appointment");
        } else {
    ...
    /**
     * Opens the NewAppointment scene using <code>SceneMovements.goToPage</code> method.
     * @param actionEvent Selecting the #toNewAppt button.
     */
    public void toNewAppt(ActionEvent actionEvent) throws IOException {

        Scenemovement.goToPage(actionEvent, "NewAppointment", 600, 550, "New Appointment");
    }
     ...
     * Refreshes the <code>Appointment</code> tableview.
     * <p>
    ...
    public void deleteAppt(ActionEvent actionEvent) {
        ...
        if (a == null) {
            ...
            alert2.setTitle("Verify Delete");
            alert2.showAndWait().ifPresent(response -> {
                if (response == ButtonType.YES) {
                    AppointmentOBJ.deleteAppointment(a.getId());
                    apptListTableView.setItems(AppointmentOBJ.getAllAppointments());
