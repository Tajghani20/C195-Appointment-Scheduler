...
/**
 * This controller class builds and populates the Customer tableview. Navigation buttons are
...
public class CustomerController implements Initializable {
    ...
    public void ToUpdateCustomer(ActionEvent actionEvent) throws IOException {
        ...
        if (c != null) {
            UpdateCustomerController.passingTheCustomer(c);
            Scenemovement.goToPage(actionEvent, "UpdateCustomer", 800, 600, "Update Customer");
        } else {
    ...
    public void ToNewCustomer(ActionEvent actionEvent) throws IOException {
        Scenemovement.goToPage(actionEvent, "NewCustomer", 800, 600, "New Customer");
    }
        ...
        if (c == null) {
            ...
            alert2.showAndWait().ifPresent(response -> {
                if (response == ButtonType.YES) {
                    if (AppointmentOBJ.checkForCustomerAppointmentLinks(c.getId()) != 0) {
                        AppointmentOBJ.deleteAppointment(AppointmentOBJ.checkForCustomerAppointmentLinks(c.getId()));
                    }
                    CustomerOBJ.deleteCustomer(c.getId());
                    customerListTable.setItems(CustomerOBJ.getAllCustomer());
    ...
    public void onActionToMainMenu(ActionEvent actionEvent) throws IOException {
        Scenemovement.goToPage(actionEvent, "MainScreen", 600, 400, "Main Menu");
    }
}